...
public class TodoController {
	...
	@GetMapping("/{id}")
	public ResponseEntity<Todo> getByTodoId(@PathVariable int id) {
	...
	public ResponseEntity<List<Todo>> getByTodoTitle(@PathVariable String title) {
		List<Todo> todo = todoSvc.getByTitle(title);
		if (todo == null)
			throw new TodoNotFoundException("Todo Not Found");
		else
	...
	@PutMapping("/{id}")
	public ResponseEntity<Todo> updateTodo(@PathVariable int id, @Valid @RequestBody Todo todo) { // modelbinding ?
																									// spring validation
																									// framework
		...
		if (newTodo != null)
			...
			// return (ResponseEntity<com.ibm.todoapp.models.Todo>)
